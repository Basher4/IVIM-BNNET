...
module tb_EvalSynthData_32lanes;
    ...
    perc_param_addr pl_i_param_idx_cached = 0;
    ...
    initial begin
        $readmemh("din_32lanes.mem", test_din);
        
        for (integer i = 0; i < 64; i+=1)
    ...
    generate
        for (i = 0; i < NUM_LANES; i += 1) begin : GEN_LANES
            PEProcessingUnit #(.IN_WIDTH(128), .PARAM_FILE($sformatf("perc_%0d_params_32lanes.mem", i))) u_Lane (
                .clk,
                .rst,
    ...
    always @(posedge clk) begin
        if (pl_o_data_vld_all) begin
            if (pl_o_data_tag == DT_TO_IL_CACHE) begin
                case (ilc_frac_part)
                    0: il_cache[il_cache_addr][ 0 : 31] = pl_o_data;
                    1: il_cache[il_cache_addr][32 : 63] = pl_o_data;
                    2: il_cache[il_cache_addr][64 : 95] = pl_o_data;
                    3: il_cache[il_cache_addr][96 :127] = pl_o_data;
                    default: $stop; 
                endcase
                
                ilc_frac_part += 1;
                if (ilc_frac_part == 4) begin
                    il_cache_addr += 1;
                    ilc_frac_part = 0;
                end
            end else begin
    ...
    initial begin
        ...
        for (voxel_idx = 0; voxel_idx < VOXELS_TO_EVALUATE; voxel_idx += 1) begin
            pl_i_param_idx = 0;
            assert (ilc_frac_part == 0) else $error("IL$ Frac part != 0");
            ...
            for (param_idx = 0; param_idx < PARAMS_TO_EVALUATE; param_idx += 1) begin
                ...
                pl_i_param_idx_cached = pl_i_param_idx; 
                #10;
                for (sample = 0; sample < SAMPLES; sample += 1) begin
                    ...
                    pl_i_data_vld = 1;
                    pl_i_param_idx = pl_i_param_idx_cached;
                    #10;
                    pl_i_param_idx += 1;
                    #10;
                    pl_i_param_idx += 1;
