...
class Net(nn.Module):
    def __init__(self, bvalues, net_pars):
        ...
        for i in range(self.net_pars.depth):
            ...
            self.fc_layers0.extend([nn.ReLU()])
            if self.net_pars.parallel != 'single':
                self.fc_layers1.extend([nn.ReLU()])
                self.fc_layers2.extend([nn.ReLU()])
                self.fc_layers3.extend([nn.ReLU()])
                if self.net_pars.tri_exp:
                    self.fc_layers4.extend([nn.ReLU()])
                    self.fc_layers5.extend([nn.ReLU()])
